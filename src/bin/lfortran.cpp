...
enum Backend { llvm, cpp, x86, wasm };
...
std::string get_kokkos_dir() {
  ...
  std::cerr << "The code C++ generated by the C++ LFortran backend uses the "
               "Kokkos library"
            << std::endl;
  std::cerr << "(https://github.com/kokkos/kokkos). Please define the "
               "LFORTRAN_KOKKOS_DIR"
            << std::endl;
  std::cerr << "environment variable to point to the Kokkos installation."
            << std::endl;
...
int emit_tokens2(const std::string &input,
                 std::vector<std::string> &tok_strings, std::vector<int> &toks,
  ...
  if (std::find(toks.begin(), toks.end(), KW_SUBROUTINE) != toks.end()) {
    ...
    for (size_t i = 0; i < toks.size(); i++) {
      if (toks[i] == KW_SUBROUTINE) {
        sr_blnc++;
      } else if (toks[i] == KW_END_SUBROUTINE || toks[i] == KW_ENDSUBROUTINE) {
        sr_blnc--;
      }
    }
  }
  ...
  if (std::find(toks.begin(), toks.end(), KW_FUNCTION) != toks.end()) {
    ...
    for (size_t i = 0; i < toks.size(); i++) {
      if (toks[i] == KW_FUNCTION) {
        fn_blnc++;
      } else if (toks[i] == KW_END_FUNCTION || toks[i] == KW_ENDFUNCTION) {
        fn_blnc--;
      }
    }
  }
  ...
  if (std::find(toks.begin(), toks.end(), KW_IF) != toks.end()) {
    ...
    for (size_t i = 0; i < toks.size(); i++) {
      if (toks[i] == KW_IF) {
        ...
        if (std::find(toks.begin() + i, toks.begin() + endif_loc, KW_THEN) !=
            toks.begin() + endif_loc) {
          ...
          for (size_t j = i + 1; j < endif_loc; j++) {
            if (toks[j] == KW_THEN) {
              ...
              for (size_t k = endif_loc; k > j; k--) {
                if (toks[k] == KW_ENDIF || toks[k] == KW_END_IF) {
                  if_blnc--;
...
int prompt(bool verbose) {
  ...
  std::cout << "    - Editing (Keys: Left, Right, Home, End, Backspace, Delete)"
            << std::endl;
  ...
  while (true) {
    ...
    try {
      ...
      lm.in_filename = "input";
      LFortran::Result<LFortran::FortranEvaluator::EvalResult> res =
          e.evaluate(input, verbose, lm, lpm, diagnostics);
      std::cerr << diagnostics.render(input, lm, cu);
      if (res.ok) {
        r = res.result;
      } else {
    ...
    switch (r.type) {
      ...
      case (LFortran::FortranEvaluator::EvalResult::complex4): {
        ...
        if (verbose) section("Result:");
        std::cout << std::setprecision(8) << "(" << r.c32.re << ", " << r.c32.im
                  << ")" << std::endl;
        break;
      }
      case (LFortran::FortranEvaluator::EvalResult::complex8): {
        ...
        if (verbose) section("Result:");
        std::cout << std::setprecision(17) << "(" << r.c64.re << ", "
                  << r.c64.im << ")" << std::endl;
        break;
      }
      ...
      default:
        throw LFortran::LCompilersException("Return type not supported");
    }
  }
  return 0;
}
...
int emit_prescan(const std::string &infile, CompilerOptions &compiler_options) {
  ...
  std::string prescan =
      LFortran::fix_continuation(input, lm, compiler_options.fixed_form);
...
int emit_ast_f90(const std::string &infile, CompilerOptions &compiler_options) {
  ...
  std::cerr << diagnostics.render(input, lm, compiler_options);
  if (r.ok) {
    std::cout << LFortran::ast_to_src(*r.result, compiler_options.use_colors);
    return 0;
  } else {
    LFORTRAN_ASSERT(diagnostics.has_error())
    return 2;
...
int format(const std::string &infile, bool inplace, bool color, int indent,
  ...
  std::string source = LFortran::ast_to_src(*ast, color, indent, indent_unit);
...
int python_wrapper(const std::string &infile, std::string array_order,
  ...
  lm.in_filename = infile;
  LFortran::diag::Diagnostics diagnostics;
  LFortran::Result<LFortran::ASR::TranslationUnit_t *> result =
      fe.get_asr2(input, lm, diagnostics);
  std::cerr << diagnostics.render(input, lm, compiler_options);
  if (result.ok) {
    asr = result.result;
  } else {
  ...
  auto pxd_fname =
      prefix +
      "_pxd.pxd";  // the "_pxd" is an ugly hack, see comment in asr_to_py.cpp
  auto pyx_fname = prefix + ".pyx";

  // The ASR to Python converter needs to know the name of the .h file that will
  // be written, but needs all path information stripped off - just the
  // filename.
  auto chdr_fname_forcodegen = chdr_fname;
  {
    ...
    auto lastpos =
        std::max((pos_windows == std::string::npos ? 0 : pos_windows),
  ...
  std::tie(c_h, pxd, pyx) =
      LFortran::asr_to_py(*asr, c_order, chdr_fname_forcodegen);
...
int emit_asr(const std::string &infile, LCompilers::PassManager &pass_manager,
  ...
  lm.in_filename = infile;
  LFortran::diag::Diagnostics diagnostics;
  LFortran::Result<LFortran::ASR::TranslationUnit_t *> r =
      fe.get_asr2(input, lm, diagnostics);
  std::cerr << diagnostics.render(input, lm, compiler_options);
  if (!r.ok) {
    LFORTRAN_ASSERT(diagnostics.has_error())
  ...
  std::cout << LFortran::pickle(*asr, compiler_options.use_colors,
                                compiler_options.indent, with_intrinsic_modules)
            << std::endl;
  return 0;
}
...
int save_mod_files(const LFortran::ASR::TranslationUnit_t &u) {
  for (auto &item : u.m_global_scope->get_scope()) {
    if (LFortran::ASR::is_a<LFortran::ASR::Module_t>(*item.second)) {
      LFortran::ASR::Module_t *m =
          LFortran::ASR::down_cast<LFortran::ASR::Module_t>(item.second);
      ...
      LFortran::ASR::asr_t *asr =
          LFortran::ASR::make_TranslationUnit_t(al, loc, symtab, nullptr, 0);
...
int compile_to_object_file(const std::string &infile,
  ...
  LFortran::Result<std::unique_ptr<LFortran::LLVMModule>> res =
      fe.get_llvm3(*asr, lpm, diagnostics);
  std::cerr << diagnostics.render(input, lm, compiler_options);
  if (res.ok) {
...
int compile_to_binary_x86(const std::string &infile, const std::string &outfile,
                          bool time_report, CompilerOptions &compiler_options) {
  ...
  {
    auto t1 = std::chrono::high_resolution_clock::now();
    LFortran::Result<LFortran::AST::TranslationUnit_t *> result =
        fe.get_ast2(input, lm, diagnostics);
    auto t2 = std::chrono::high_resolution_clock::now();
    time_src_to_ast =
        std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
  ...
  if (time_report) {
    ...
    std::cout << "Src -> AST:  " << std::setw(5) << time_src_to_ast
              << std::endl;
    std::cout << "AST -> ASR:  " << std::setw(5) << time_ast_to_asr
              << std::endl;
    std::cout << "ASR -> x86:  " << std::setw(5) << time_asr_to_x86
              << std::endl;
    int total =
        time_file_read + time_src_to_ast + time_ast_to_asr + time_asr_to_x86;
...
int compile_to_binary_wasm(const std::string &infile,
  ...
  // ASR -> wasm machine code
  {
    ...
    auto t1 = std::chrono::high_resolution_clock::now();
    LFortran::Result<int> result =
        LFortran::asr_to_wasm(*asr, al, outfile, time_report, diagnostics);
    auto t2 = std::chrono::high_resolution_clock::now();
    time_asr_to_wasm =
        std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
  ...
  if (time_report) {
    ...
    std::cout << "AST -> ASR:  " << std::setw(5) << time_ast_to_asr
              << std::endl;
    std::cout << "ASR -> wasm:  " << std::setw(5) << time_asr_to_wasm
              << std::endl;
    int total =
        time_file_read + time_src_to_ast + time_ast_to_asr + time_asr_to_wasm;
    std::cout << "Total:       " << std::setw(5) << total << std::endl;
  }
...
int compile_to_object_file_cpp(const std::string &infile,
                               const std::string &outfile, bool assembly,
                               bool kokkos, const std::string &rtlib_header_dir,
                               CompilerOptions &compiler_options) {
  std::string input = read_file(infile);
  ...
  std::string src;
  diagnostics.diagnostics.clear();
  LFortran::Result<std::string> res = fe.get_cpp2(*asr, diagnostics, 1);
  std::cerr << diagnostics.render(input, lm, compiler_options);
  if (res.ok) {
    src = res.result;
  } else {
  ...
  */
...
#ifdef HAVE_LFORTRAN_LLVM
  std::string t = (compiler_options.target == "")
                      ? LFortran::LLVMEvaluator::get_default_target_triple()
                      : compiler_options.target;
#else
  std::string t = (compiler_options.platform == LFortran::Platform::Windows)
  ...
  if (backend == Backend::llvm) {
    if (t == "x86_64-pc-windows-msvc") {
      ...
      cmd += +" -L" + base_path + " -Wl,-rpath," + base_path + " -l" +
             runtime_lib + " -lm";
      int err = system(cmd.c_str());
      if (err) {
        std::cout << "The command '" + cmd + "' failed." << std::endl;
    ...
    if (kokkos) {
      std::string kokkos_dir = get_kokkos_dir();
      post_options += kokkos_dir + "/lib/libkokkoscontainers.a " + kokkos_dir +
                      "/lib/libkokkoscore.a -ldl";
    }
    ...
    std::string cmd = "cp " + infiles[0] + " " + outfile + " && " + "cp " +
                      infiles[0] + ".js" + " " + outfile + ".js";
    int err = system(cmd.c_str());
    if (err) {
      std::cout << "The command '" + cmd + "' failed." << std::endl;
      return 10;
...
int emit_c_preprocessor(const std::string &infile,
                        CompilerOptions &compiler_options) {
  std::string input = read_file(infile);
...
EMSCRIPTEN_KEEPALIVE char *emit_wasm_from_source(char *input) {
  INITIALIZE_VARS;
  LFortran::Result<LFortran::Vec<uint8_t>> r =
      fe.get_wasm(input, lm, diagnostics);
  if (r.ok) {
    out = "0";  // exit code
...
int main(int argc, char *argv[]) {
  ...
  try {
    int dirname_length;
    LFortran::get_executable_path(LFortran::binary_executable_path,
                                  dirname_length);
    ...
    app.add_option("-D", compiler_options.c_preprocessor_defines,
                   "Define <macro>=<value> (or 1 if <value> omitted)")
        ->allow_extra_args(false);
    app.add_flag("--version", arg_version,
                 "Display compiler version information");

    // LFortran specific options
    app.add_flag("--cpp", compiler_options.c_preprocessor,
                 "Enable C preprocessing");
    app.add_flag("--fixed-form", compiler_options.fixed_form,
                 "Use fixed form Fortran source parsing");
    app.add_flag("--show-prescan", show_prescan,
                 "Show tokens for the given file and exit");
    app.add_flag("--show-tokens", show_tokens,
                 "Show tokens for the given file and exit");
    app.add_flag("--show-ast", show_ast,
                 "Show AST for the given file and exit");
    app.add_flag("--show-asr", show_asr,
                 "Show ASR for the given file and exit");
    app.add_flag("--with-intrinsic-mods", with_intrinsic_modules,
                 "Show intrinsic modules in ASR");
    app.add_flag("--show-ast-f90", show_ast_f90,
                 "Show Fortran from AST for the given file and exit");
    ...
    app.add_option("--pass", arg_pass,
                   "Apply the ASR pass and show ASR (implies --show-asr)");
    app.add_flag("--show-llvm", show_llvm,
                 "Show LLVM IR for the given file and exit");
    app.add_flag("--show-cpp", show_cpp,
                 "Show C++ translation source for the given file and exit");
    app.add_flag("--show-c", show_c,
                 "Show C translation source for the given file and exit");
    app.add_flag("--show-asm", show_asm,
                 "Show assembly for the given file and exit");
    app.add_flag("--show-wat", show_wat,
                 "Show WAT (WebAssembly Text Format) and exit");
    app.add_flag("--show-stacktrace", compiler_options.show_stacktrace,
                 "Show internal stacktrace on compiler errors");
    app.add_flag("--symtab-only", compiler_options.symtab_only,
                 "Only create symbol tables in ASR (skip executable stmt)");
    ...
    app.add_flag("--no-warnings", compiler_options.no_warnings,
                 "Turn off all warnings");
    app.add_flag("--no-error-banner", compiler_options.no_error_banner,
                 "Turn off error banner");
    app.add_option("--error-format", compiler_options.error_format,
                   "Control how errors are produced (human, short)")
        ->capture_default_str();
    app.add_option("--backend", arg_backend,
                   "Select a backend (llvm, cpp, x86, wasm)")
        ->capture_default_str();
    app.add_flag("--openmp", compiler_options.openmp, "Enable openmp");
    app.add_flag("--generate-object-code",
                 compiler_options.generate_object_code,
                 "Generate object code into .o files");
    app.add_flag("--fast", compiler_options.fast,
                 "Best performance (disable strict standard compliance)");
    app.add_flag("--link-with-gcc", link_with_gcc,
                 "Calls GCC for linking instead of clang");
    app.add_option("--target", compiler_options.target,
                   "Generate code for the given target")
        ->capture_default_str();
    app.add_flag("--print-targets", print_targets,
                 "Print the registered targets");
    app.add_flag("--implicit-typing", compiler_options.implicit_typing,
                 "Allow implicit typing");
    app.add_flag("--allow-implicit-interface",
                 compiler_options.implicit_interface,
                 "Allow implicit interface");
    ...
    fmt.add_option("file", arg_fmt_file, "Fortran source file to format")
        ->required();
    fmt.add_flag("-i", arg_fmt_inplace,
                 "Modify <file> in-place (instead of writing to stdout)");
    fmt.add_option("--spaces", arg_fmt_indent,
                   "Number of spaces to use for indentation")
        ->capture_default_str();
    fmt.add_flag("--indent-unit", arg_fmt_indent_unit,
                 "Indent contents of sub / fn / prog / mod");
    fmt.add_flag("--no-color", arg_fmt_no_color,
                 "Turn off color when writing to stdout");

    // kernel
    CLI::App &kernel =
        *app.add_subcommand("kernel", "Run in Jupyter kernel mode.");
    kernel.add_option("-f", arg_kernel_f, "The kernel connection file")
        ->required();
    ...
    // pywrap
    CLI::App &pywrap =
        *app.add_subcommand("pywrap", "Python wrapper generator");
    pywrap.add_option("file", arg_pywrap_file, "Fortran source file (*.f90)")
        ->required();
    pywrap
        .add_option("--array-order", arg_pywrap_array_order,
                    "Select array order (c, f)")
        ->capture_default_str();
    ...
    if (arg_version) {
      ...
      switch (compiler_options.platform) {
        case (LFortran::Platform::Linux):
          std::cout << "Linux";
          break;
        case (LFortran::Platform::macOS_Intel):
          std::cout << "macOS Intel";
          break;
        case (LFortran::Platform::macOS_ARM):
          std::cout << "macOS ARM";
          break;
        case (LFortran::Platform::Windows):
          std::cout << "Windows";
          break;
        case (LFortran::Platform::FreeBSD):
          std::cout << "FreeBSD";
          break;
      }
      std::cout << std::endl;
    ...
    if (print_targets) {
      ...
      return 0;
#else
      std::cerr << "The --print-targets option requires the LLVM backend to be "
                   "enabled. Recompile with `WITH_LLVM=yes`."
                << std::endl;
      return 1;
#endif
    }
    ...
    if (fmt) {
      if (CLI::NonexistentPath(arg_fmt_file).empty())
        throw LFortran::LCompilersException("File does not exist: " +
                                            arg_fmt_file);
...
#else
      std::cerr << "The kernel subcommand requires LFortran to be compiled "
                   "with XEUS support. Recompile with `WITH_XEUS=yes`."
                << std::endl;
      return 1;
#endif
    }
    ...
    if (arg_backend == "llvm") {
      ...
      std::cerr << "The backend must be one of: llvm, cpp, x86, wasm."
                << std::endl;
      return 1;
    }
    ...
    if (arg_files.size() == 0) {
      ...
      return prompt(arg_v);
#else
      std::cerr << "Interactive prompt requires the LLVM backend to be "
                   "enabled. Recompile with `WITH_LLVM=yes`."
                << std::endl;
      return 1;
#endif
    ...
    if (show_asr) {
      return emit_asr(arg_file, lfortran_pass_manager, with_intrinsic_modules,
                      compiler_options);
    }
    lfortran_pass_manager.use_default_passes();
    if (show_llvm) {
#ifdef HAVE_LFORTRAN_LLVM
      return emit_llvm(arg_file, lfortran_pass_manager, compiler_options);
#else
      std::cerr << "The --show-llvm option requires the LLVM backend to be "
                   "enabled. Recompile with `WITH_LLVM=yes`."
                << std::endl;
      return 1;
#endif
...
#else
      std::cerr << "The --show-asm option requires the LLVM backend to be "
                   "enabled. Recompile with `WITH_LLVM=yes`."
                << std::endl;
      return 1;
#endif
...
#else
        std::cerr << "The -S option requires the LLVM backend to be enabled. "
                     "Recompile with `WITH_LLVM=yes`."
                  << std::endl;
        return 1;
#endif
      } else if (backend == Backend::cpp) {
        std::cerr << "The C++ backend does not work with the -S option yet."
                  << std::endl;
        return 1;
      } else {
        LFORTRAN_ASSERT(false);
    ...
    if (arg_c) {
      if (backend == Backend::llvm) {
        ...
        return compile_to_object_file(arg_file, outfile, false,
                                      compiler_options);
#else
        std::cerr << "The -c option requires the LLVM backend to be enabled. "
                     "Recompile with `WITH_LLVM=yes`."
                  << std::endl;
        return 1;
#endif
        ...
        return compile_to_object_file_cpp(arg_file, outfile, false, true,
                                          rtlib_header_dir, compiler_options);
      } else if (backend == Backend::x86) {
        return compile_to_binary_x86(arg_file, outfile, time_report,
                                     compiler_options);
      } else if (backend == Backend::wasm) {
        return compile_to_binary_wasm(arg_file, outfile, time_report,
                                      compiler_options);
      } else {
    ...
    if (endswith(arg_file, ".f90")) {
      ...
      if (backend == Backend::llvm) {
#ifdef HAVE_LFORTRAN_LLVM
        err = compile_to_object_file(arg_file, tmp_o, false, compiler_options);
#else
        std::cerr
            << "Compiling Fortran files to object files requires the LLVM "
               "backend to be enabled. Recompile with `WITH_LLVM=yes`."
            << std::endl;
        return 1;
#endif
      } else if (backend == Backend::cpp) {
      ...
      if (err) return err;
      return link_executable({tmp_o}, outfile, runtime_library_dir, backend,
                             static_link, link_with_gcc, true,
                             compiler_options);
    } else {
      return link_executable(arg_files, outfile, runtime_library_dir, backend,
                             static_link, link_with_gcc, true,
                             compiler_options);
    }
  } catch (const LFortran::LCompilersException &e) {
