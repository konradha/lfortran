...
class VecIterator {
 public:
  VecIterator(const Vec<T>& c, size_t idx = 0) : m_container(c), m_index(idx) {}
...
struct Vec {
  ...
  const T& operator[](size_t pos) const { return p[pos]; }

  // Returns a copy of the data as std::vector
  std::vector<T> as_vector() const { return std::vector<T>(p, p + n); }
  ...
  VecIterator<T> begin() const { return VecIterator<T>(*this, 0); }

  VecIterator<T> end() const { return VecIterator<T>(*this, n); }
};
