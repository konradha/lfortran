...
std::string strop2str(const AST::stroperatorType type) {
  switch (type) {
    case (AST::stroperatorType::Concat):
      return " // ";
  }
  throw LCompilersException("Unknown type");
}
...
std::string intrinsicop2str(const AST::intrinsicopType type) {
  switch (type) {
    case (AST::intrinsicopType::AND):
      return ".and.";
    case (AST::intrinsicopType::OR):
      return ".or.";
    case (AST::intrinsicopType::XOR):
      return ".xor.";
    case (AST::intrinsicopType::EQV):
      return ".eqv.";
    case (AST::intrinsicopType::NEQV):
      return ".neqv.";
    case (AST::intrinsicopType::PLUS):
      return "+";
    case (AST::intrinsicopType::MINUS):
      return "-";
    case (AST::intrinsicopType::STAR):
      return "*";
    case (AST::intrinsicopType::DIV):
      return "/";
    case (AST::intrinsicopType::POW):
      return "**";
    case (AST::intrinsicopType::NOT):
      return ".not.";
    case (AST::intrinsicopType::EQ):
      return "==";
    case (AST::intrinsicopType::GT):
      return ">";
    case (AST::intrinsicopType::GTE):
      return ">=";
    case (AST::intrinsicopType::LT):
      return "<";
    case (AST::intrinsicopType::LTE):
      return "<=";
    case (AST::intrinsicopType::NOTEQ):
      return "/=";
    case (AST::intrinsicopType::CONCAT):
      return "//";
  }
  throw LCompilersException("Unknown type");
}
...
std::string symbol2str(const AST::symbolType type) {
  switch (type) {
    case (AST::symbolType::None):
      return "";
    case (AST::symbolType::Arrow):
      return " => ";
    case (AST::symbolType::Equal):
      return " = ";
    case (AST::symbolType::Asterisk):
      return "*";
    case (AST::symbolType::DoubleAsterisk):
      return "*(*)";
    case (AST::symbolType::Slash):
      return "/";
  }
  throw LCompilersException("Unknown type");
}
...
class ASTToSRCVisitor : public BaseVisitor<ASTToSRCVisitor> {
 ...
 public:
      ...
      for (size_t i = 0; i < y.n_inside; i++) {
        switch (y.m_inside[i]->type) {
          case trivia_nodeType::Comment: {
            r += std::string(down_cast<Comment_t>(y.m_inside[i])->m_comment);
            r += "\n";
            break;
          }
          case trivia_nodeType::EOLComment: {
            r += " ";
            r += std::string(down_cast<EOLComment_t>(y.m_inside[i])->m_comment);
            r += "\n";
            break;
          }
    ...
    if (y.n_after > 0) {
      for (size_t i = 0; i < y.n_after; i++) {
        switch (y.m_after[i]->type) {
          case trivia_nodeType::Comment: {
            r += std::string(down_cast<Comment_t>(y.m_after[i])->m_comment);
            r += "\n";
            break;
          }
          case trivia_nodeType::EOLComment: {
            r += " ";
            r += std::string(down_cast<EOLComment_t>(y.m_after[i])->m_comment);
            r += "\n";
            break;
          }
          case trivia_nodeType::EndOfLine: {
  ...
  void visit_InterfaceHeader(const InterfaceHeader_t & /* x */) { s = ""; }
  ...
  std::string format_import(const Program_t & /*x*/) { return ""; }

  std::string format_import(const Module_t & /*x*/) { return ""; }
  ...
  std::string format_body(const Module_t & /*x*/) { return ""; }
  ...
  void visit_ImplicitNoneType(const ImplicitNoneType_t & /*x*/) { s = "type"; }
  ...
  void visit_Declaration(const Declaration_t &x) {
    ...
    if (x.m_vartype == nullptr && x.n_attributes == 1 &&
      ...
      for (size_t i = 0; i < x.n_attributes; i++) {
        if (x.m_attributes[i]->type == decl_attributeType::AttrData && i == 0) {
          r += syn(gr::Type);
          r += "data ";
          r += syn();
        }
...
#define ATTRTYPE2(x, y)          \
  ...
  void visit_AttrBind(const AttrBind_t &x) { visit_bind(*x.m_bind); }
    ...
    for (size_t i = 0; i < x.n_member; i++) {
      ...
      for (size_t j = 0; j < x.m_member[i].n_args; j++) {
        ...
        if (step != nullptr) {
          ...
          if (is_a<Num_t>(*step) && down_cast<Num_t>(step)->m_n != 1) {
            r += ":";
