...
std::vector<std::string> declarators{
    "integer",   "real",      "complex", "doubleprecision", "external",
    "dimension", "character", "logical", "bytes",           "data"};
  ...
  */
  ...
  void tokenize_line(const std::string &chop, unsigned char *&cur) {
    ...
    if (start >= cur) {
      ...
      throw LFortran::parser_local::TokenizerError(
          "ICE: chop longer than a line: '" + chop + "'", loc);
    }
    ...
    for (;;) {
      ...
      if (next_is(t.tok, "goto") && token != yytokentype::KW_GOTO) {
        ...
        YYSTYPE y3;
        lex_int_large(m_a, t.tok + 4, t.cur, y3.int_suffix.int_n,
  ...
  bool lex_if_statement(unsigned char *&cur) {
    ...
    for (;;) {
      ...
      if (match_levels == 0 &&
          tokens[tokens.size() - 1] == yytokentype::TK_RPAREN &&
          next_is(t.cur, "call")) {
        YYSTYPE y3;
        std::string l("call");
  ...
  bool lex_do_terminal(unsigned char *&cur, int64_t do_label) {
    ...
    if (next_is(cur, "enddo")) {
      ...
      for (int i = 0; i < do_levels; ++i) insert_enddo();
  ...
  */
  ...
  void lex_do(unsigned char *&cur) {
    ...
    while (!lex_do_terminal(cur, do_label)) {
      if (!lex_body_statement(cur)) {
        throw parser_local::TokenizerError("End of file inside a do loop 2",
                                           loc);
      };
  ...
  void lex_global_scope_item(unsigned char *&cur) {
    ...
    std::vector<std::string> function_keywords{
        "recursive", "pure",    "elemental", "real",           "character",
        "complex",   "integer", "logical",   "doubleprecision"};
