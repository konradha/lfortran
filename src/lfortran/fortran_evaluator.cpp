...
Result<FortranEvaluator::EvalResult> FortranEvaluator::evaluate2(
...
Result<FortranEvaluator::EvalResult> FortranEvaluator::evaluate(
  ...
  // ASR -> LLVM
  Result<std::unique_ptr<LLVMModule>> res3 =
      get_llvm3(*asr, pass_manager, diagnostics);
  ...
  if (return_type == "integer4") {
    ...
    throw LCompilersException(
        "FortranEvaluator::evaluate(): Return type not supported");
  }
  return result;
#else
...
Result<AST::TranslationUnit_t *> FortranEvaluator::get_ast2(
  ...
  Result<AST::TranslationUnit_t *> res =
      parse(al, *code, diagnostics, compiler_options.fixed_form);
  if (res.ok) {
    return res.result;
  } else {
    LFORTRAN_ASSERT(diagnostics.has_error())
...
Result<std::string> FortranEvaluator::get_asr(const std::string &code,
  ...
  if (asr.ok) {
    return LFortran::pickle(*asr.result, compiler_options.use_colors,
                            compiler_options.indent);
  } else {
    LFORTRAN_ASSERT(diagnostics.has_error())
    return asr.error;
...
Result<std::string> FortranEvaluator::get_llvm(
    ...
    LCompilers::PassManager &pass_manager, diag::Diagnostics &diagnostics) {
  Result<std::unique_ptr<LLVMModule>> res =
      get_llvm2(code, lm, pass_manager, diagnostics);
  if (res.ok) {
#ifdef HAVE_LFORTRAN_LLVM
...
Result<std::unique_ptr<LLVMModule>> FortranEvaluator::get_llvm2(
  ...
  Result<std::unique_ptr<LLVMModule>> res =
      get_llvm3(*asr.result, pass_manager, diagnostics);
  if (res.ok) {
#ifdef HAVE_LFORTRAN_LLVM
...
Result<std::unique_ptr<LLVMModule>> FortranEvaluator::get_llvm3(
  ...
  Result<std::unique_ptr<LFortran::LLVMModule>> res =
      asr_to_llvm(asr, diagnostics, e->get_context(), al, pass_manager,
...
#ifdef HAVE_LFORTRAN_LLVM
    const std::string &code, LocationManager &lm, LCompilers::PassManager &lpm,
    diag::Diagnostics &diagnostics
#else
    const std::string & /*code*/, LocationManager & /*lm*/,
...
#ifdef HAVE_LFORTRAN_LLVM
  Result<std::unique_ptr<LLVMModule>> res =
      get_llvm2(code, lm, lpm, diagnostics);
  if (res.ok) {
...
Result<Vec<uint8_t>> FortranEvaluator::get_wasm(
    const std::string &code, LocationManager &lm,
    diag::Diagnostics &diagnostics) {
  // Src -> AST -> ASR -> WASM
